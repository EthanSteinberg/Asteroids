...
bool t_game::moveall()
{
   ...
   if (Input->IsKeyDown(sf::Key::Space) && boost::posix_time::microsec_clock::universal_time() > lastPushed)
      bullets.push_back(ship.shoot()),lastPushed = boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(1000);
   
   for (unsigned int i = 0;i <bullets.size();i++)
      bullets[i]->move(boost::posix_time::microsec_clock::universal_time() -time);
